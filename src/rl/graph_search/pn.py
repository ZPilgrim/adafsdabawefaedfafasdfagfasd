...
class GraphSearchPolicy(nn.Module):
    ...
    def transit_on_abs(self, e, obs, kg, use_action_space_bucketing=False, merge_aspace_batching_outcome=False):
        """
        Compute the next action distribution based on
            (a) the current node (entity) in KG and the query relation
            (b) action history representation
    ...
    def transit_same(self, e, obs, e_abs, obs_abs, kg, use_action_space_bucketing=True, \
        ...
        if self.relation_only:
            ...
            E_s = kg.get_entity_abs_embeddings(e_s_abs)
            E = kg.get_entity_abs_embeddings(e_abs)
            X = torch.cat([E, H, E_s, Q], dim=-1)
        else:  # 应该走的这个分支
            E = kg.get_entity_abs_embeddings(e_abs)
            X = torch.cat([E, H, Q], dim=-1)
    ...
    def get_action_space_e2t_t2e(self,e, obs, e_abs, obs_abs, kg):
        if not kg.action_space_e2t or not kg.action_space:
            print("NONE:", kg.action_space_e2t, kg.action_space)

        r_space, e_space_abs = kg.action_space_e2t[0][0][e], kg.action_space_e2t[0][1][e]

        e_space = kg.action_space[0][1][e]
        action_mask_abs = kg.action_space_e2t[1][e]
        action_mask =  kg.action_space[1][e]
        action_space_abs = ((r_space, e_space_abs), action_mask_abs)
        action_space = ((r_space, e_space), action_mask)
