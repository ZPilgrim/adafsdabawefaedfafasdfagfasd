...
def merge_same(log_action_prob, next_e, next_r, method):
    ...
    for i in range(len(log_action_prob)):
        ...
        for e, p, idx, r in zip(next_e[i].tolist(), log_action_prob[i].tolist(), action_ind, next_r[i].tolist()):
            if e not in esi:
                esi[e] = [[p], idx, r]
            else:
                if p > max(esi[e][0]):
    ...
    merged_prob, merged_next_e, merged_ind, merged_r = torch.Tensor(merged_prob).cuda(), torch.LongTensor(
        merged_next_e).cuda(), torch.LongTensor(merged_ind).cuda(), torch.LongTensor(merged_r).cuda()
